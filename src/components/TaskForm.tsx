...
export default function TaskForm() {
  ...
  const onSubmit = (data: FormData) => {
    ...
    if (data.dueDate) {
      const formatted =
        data.dueDate.length === 16 ? data.dueDate + ":00" : data.dueDate; // append seconds if missing
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-4 rounded shadow space-y-3">
      ...
      <input {...register("title")} placeholder="Task title" className="border p-2 w-full" />
      ...
      <input {...register("assignedToId")} placeholder="Assign to User ID" className="border p-2 w-full" />
      {errors.assignedToId && <p className="text-red-500 text-sm">Assigned user is required</p>}

      <button type="submit" disabled={isLoading} className="bg-blue-600 text-white py-2 rounded w-full">
